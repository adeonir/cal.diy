...
type AvailableTimeSlotsProps = {
  ...
  isOutsideBusinessHours: (slotStartIso?: string | null) => boolean;
...
export const AvailableTimeSlots = ({
  ...
  const onTimeSelect = useCallback(
    (time: string, attendees: number, seatsPerTimeSlot?: number | null, bookingUid?: string) => {
      ...
      if (skipConfirmStep && isTimeSlotAvailable && !isOutsideBusinessHours(time)) {
        onSubmit(time);
      }
      return;
    },
  ...
  const handleSlotClick = useCallback(
    (selectedSlot: Slot, isOverlapping: boolean) => {
      if (
        (overlayCalendarToggled && isOverlapping) ||
        (skipConfirmStep && !isOutsideBusinessHours(selectedSlot.time))
      ) {
